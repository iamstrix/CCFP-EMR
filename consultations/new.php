    ...
    <div class="form-grid">
      ...
      <div class="form-group" style="grid-column: span 2;">
        ...
        <select name="patient_id" id="patient_id" required>
          ...
          <?php foreach ($patients as $p): ?>
            <option value="<?= $p['patient_id'] ?>" <?= ((int) ($_POST['patient_id'] ?? $prefillPat) === (int) $p['patient_id']) ? 'selected' : '' ?>>
      ...
      <div class="form-group">
        ...
        <input type="date" name="visit_date" id="visit_date"
          value="<?= htmlspecialchars($_POST['visit_date'] ?? date('Y-m-d')) ?>" max="<?= date('Y-m-d') ?>" required>
      </div>
      ...
      <div class="form-group">
        ...
        <select name="physician_id" id="physician_id">
          ...
          <?php foreach ($physicians as $ph): ?>
            <option value="<?= $ph['physician_id'] ?>" <?= (($_POST['physician_id'] ?? '') == $ph['physician_id']) ? 'selected' : '' ?>>
      ...
      <div class="form-group" style="grid-column: span 2;">
          ...
          <div class="autocomplete-wrapper">
            <input type="text" name="chief_complaint" id="chief_complaint" class="form-control" autocomplete="off"
              required>
          </div>
      </div>
      ...
      <div class="form-group" style="grid-column: span 2;">
          ...
          <div class="autocomplete-wrapper">
            <input type="text" name="diagnosis" id="diagnosis" class="form-control" autocomplete="off" required>
          </div>
      </div>
      ...
      <div class="form-group" style="grid-column: span 2;">
        ...
        <textarea name="treatment" id="treatment" rows="3"
          placeholder="Dosage, instructions, etc."><?= htmlspecialchars($_POST['treatment'] ?? '') ?></textarea>
      </div>
