...
if ($mode === 'patient') {
    ...
    $s2 = $pdo->prepare(
        "SELECT c.*, cat.name AS cat_name, CONCAT(ph.last_name,', ',ph.first_name) AS physician
         FROM consultation c
...
if ($mode === 'summary') {
    $s = $pdo->prepare(
        "SELECT c.visit_date,
                ...
                cat.name AS category, c.chief_complaint, c.diagnosis,
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .report-page { background: #fff; max-width: 900px; margin: 1.5rem auto; padding: 2rem; box-shadow: 0 0 16px rgba(0,0,0,.12); }
...
<div class="report-page">
  ...
  <div class="info-grid">
    <div class="info-item">
      ...
      <span><?= htmlspecialchars($patient['last_name'].', '.$patient['first_name'].($patient['middle_name'] ? ' '.$patient['middle_name'] : '')) ?></span>
    </div>
    <div class="info-item">
    ...
    <div class="info-item" style="grid-column:1/-1;">
      <label>Address</label>
      <span><?= htmlspecialchars($patient['street_address'].', '.$patient['barangay'].', '.$patient['municipality']) ?></span>
    </div>
    <div class="info-item">
      <label>Demographics</label>
      <span>
        <?= $patient['is_ip']   ? '<span class="badge badge-ip">IP</span> ' : '' ?>
        <?= $patient['is_nhts'] ? '<span class="badge badge-nhts">NHTS</span>' : '' ?>
        <?= (!$patient['is_ip'] && !$patient['is_nhts']) ? '—' : '' ?>
      </span>
    </div>
    <div class="info-item">
  ...
  <table>
    <thead>
      <tr><th>Date</th><th>Category</th><th>Physician</th><th>Chief Complaint</th><th>Diagnosis</th><th>Treatment</th><th>Follow-up</th></tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td><?= htmlspecialchars(mb_strimwidth($c['treatment'] ?? '—', 0, 60, '…')) ?></td>
        <td><?= $c['follow_up_date'] ? htmlspecialchars($c['follow_up_date']) : '—' ?><?= $c['is_referred'] ? ' <span class="badge badge-nhts">REF</span>' : '' ?></td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
  ...
  <table>
    <thead>
      <tr><th>Date</th><th>Patient</th><th>Age</th><th>Sex</th><th>Brgy</th><th>Tag</th><th>Category</th><th>Complaint</th><th>Diagnosis</th><th>Physician</th></tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td>
          <?= $r['is_ip']   ? '<span class="badge badge-ip">IP</span> ' : '' ?>
          <?= $r['is_nhts'] ? '<span class="badge badge-nhts">4Ps</span>' : '' ?>
        </td>
